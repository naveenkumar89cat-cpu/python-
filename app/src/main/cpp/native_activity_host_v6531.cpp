...
#define LOG_TAG "RobotAdminV6531"
...
std::atomic<unsigned long> g_probe_count{0};
...
bool backend_check(){
    ...
    if(rc!=0){
        ...
        if(poll(&p,1,700)<=0){g_diag=2;close(fd);return false;}
        int e=0; socklen_t l=sizeof(e); getsockopt(fd,SOL_SOCKET,SO_ERROR,&e,&l);
        if(e){g_diag=2;close(fd);return false;}
    }
    ...
    for(;;){ssize_t n=recv(fd,b,sizeof(b),0);if(n<=0)break;s.append(b,(size_t)n);if(s.size()>65536)break;}
    close(fd);
    if(s.empty()){g_diag=4;return false;}
    bool http_ok=s.find("HTTP/1.1 200")!=std::string::npos || s.find("HTTP/1.0 200")!=std::string::npos;
    bool pass_ok=s.find("\"status\":\"PASS\"")!=std::string::npos || s.find("\"status\": \"PASS\"")!=std::string::npos;
    bool ok=http_ok && pass_ok;
    g_diag=ok?200:5; return ok;
}
...
bool render(ANativeWindow*w){
 ...
 int s=W/220;if(s<2)s=2;if(s>6)s=6; int st=g_backend_state.load(),dg=g_diag.load(); float r=st==1?.2f:.95f,g=st==1?.9f:.65f,b=st==1?.35f:.25f;
 text("ROBOT ADMIN AI OS",8*s,H-25*s,s,.92f,.96f,1); text("VERSION: 65.3.1",8*s,H-39*s,s,.6f,.75f,.95f);
 text("CPP CORE: READY",8*s,H-57*s,s,.35f,.9f,.55f); text("MODE: OFFLINE FIRST",8*s,H-71*s,s,.75f,.85f,.95f);
 text("PY BACKEND:",8*s,H-89*s,s,.92f,.96f,1); text(st==1?"CONNECTED":(st==-1?"OPTIONAL OFF":"CHECKING"),78*s,H-89*s,s,r,g,b); text("DIAG: "+std::to_string(dg),8*s,H-103*s,s,.8f,.8f,.9f);
 text("PROBE OK: "+std::to_string(g_probe_ok.load()),8*s,H-117*s,s,.55f,.8f,.95f); text("PROBE FAIL: "+std::to_string(g_probe_fail.load()),78*s,H-117*s,s,.55f,.8f,.95f);
 text("AUTO RECHECK: ON",8*s,H-131*s,s,.55f,.8f,.95f); text("SAFETY: LOCKED",8*s,H-149*s,s,.35f,.9f,.55f); text("WRITE DELETE RESTORE: OFF",8*s,H-163*s,s,.95f,.55f,.3f);
 text("LIVE ORDERS: OFF",8*s,H-177*s,s,.95f,.55f,.3f); text("AUTO UPDATE: OFF",8*s,H-191*s,s,.95f,.55f,.3f); text("PRODUCTION GATE: CLOSED",8*s,H-205*s,s,.95f,.55f,.3f); text("EMERGENCY STOP: READY",8*s,H-219*s,s,.35f,.9f,.55f);
...
void redraw(){std::lock_guard<std::mutex>l(g_mutex);if(g_window)render(g_window);}
void probe_loop(){
 while(g_alive){
   g_probe_count++; bool ok=backend_check(); if(ok)g_probe_ok++; else g_probe_fail++; int old=g_backend_state.exchange(ok?1:-1); if(old!=g_backend_state.load())redraw();
   for(int i=0;i<50&&g_alive;i++)std::this_thread::sleep_for(std::chrono::milliseconds(100));
 }
 g_probe_running=false;
}
void start_probe(){bool expected=false;if(g_alive&&g_probe_running.compare_exchange_strong(expected,true))std::thread(probe_loop).detach();}
void on_start(ANativeActivity*){g_alive=true;start_probe();}
void on_resume(ANativeActivity*){redraw();start_probe();}
void on_pause(ANativeActivity*){}
void on_stop(ANativeActivity*){}
void wc(ANativeActivity*,ANativeWindow*w){{std::lock_guard<std::mutex>l(g_mutex);if(g_window)ANativeWindow_release(g_window);g_window=w;if(w)ANativeWindow_acquire(w);}redraw();start_probe();}
void wr(ANativeActivity*,ANativeWindow*){redraw();}
void wd(ANativeActivity*,ANativeWindow*w){std::lock_guard<std::mutex>l(g_mutex);if(g_window==w&&g_window){ANativeWindow_release(g_window);g_window=nullptr;}}
void destroy(ANativeActivity*){g_alive=false;std::lock_guard<std::mutex>l(g_mutex);if(g_window){ANativeWindow_release(g_window);g_window=nullptr;}}
}
extern "C" const char* ra6531_version(){return"65.3.1";}
extern "C" int ra6531_core_ready(){return 1;}
extern "C" const char* ra6531_mode(){return"OFFLINE_FIRST";}
extern "C" const char* ra6531_safety(){return"LOCKED";}
extern "C" int ra6531_write_allowed(){return 0;}
extern "C" int ra6531_live_orders_allowed(){return 0;}
extern "C" int ra6531_auto_update_allowed(){return 0;}
extern "C" int ra6531_production_gate_open(){return 0;}
extern "C" int ra6531_write(){return-403;}
extern "C" int ra6531_apply(){return-403;}
extern "C" int ra6531_restore(){return-403;}
extern "C" int ra6531_install(){return-403;}
extern "C" int ra6531_delete(){return-403;}
extern "C" int ra6531_repair(){return-403;}
extern "C" int ra6531_live_order(){return-403;}
