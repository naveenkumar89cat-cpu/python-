...
struct FastState687 {
    ...
    bool asset_tasks = false;
    bool asset_csv = false;
    bool asset_verify = false;
};
...
static void load687(ANativeActivity* a) {
    ...
    st687.asset_tasks = ra680::read_asset(m, "v673/tasks.txt", tasks_text);
    st687.asset_csv = ra680::read_asset(m, "v673/sample.csv", csv_text);
    st687.asset_verify = ra680::read_asset(m, "v673/verify.txt", verify_text);

    std::vector<ra681::FileItem> file_items;
    if (st687.asset_tasks) file_items.push_back({"tasks.txt", "v673/tasks.txt", static_cast<std::uint64_t>(tasks_text.size()), false, true, "txt"});
    if (st687.asset_csv) file_items.push_back({"sample.csv", "v673/sample.csv", static_cast<std::uint64_t>(csv_text.size()), false, true, "csv"});
    if (st687.asset_verify) file_items.push_back({"verify.txt", "v673/verify.txt", static_cast<std::uint64_t>(verify_text.size()), false, true, "txt"});
    ...
    const bool file_ok = st687.files.total == 3 && ra681_file_write_blocked() == -403 && ra681_file_delete_blocked() == -403;
    const bool task_ok = st687.tasks.total == 3 && st687.tasks.invalid == 0 && ra682_task_execute_blocked() == -403;
    const bool data_ok = st687.data.rows > 0 && st687.data.invalid_rows == 0 && ra683_data_write_blocked() == -403;
    const bool backup_ok = st687.backup.verified == 1 && st687.backup.mismatch == 0 && ra684_restore_blocked() == -403;
    const bool code_ok = st687.code.total_files == 7 && ra686_code_modify_blocked() == -403 && ra686_compile_execute_blocked() == -403;
    ...
    st687.regression = ra687::run_core_regression(
        ...
        ra681_file_write_blocked() == -403 && ra683_data_write_blocked() == -403,
        ra681_file_delete_blocked() == -403 && ra682_task_delete_blocked() == -403,
        ra684_restore_blocked() == -403,
        ra684_apply_rollback_blocked() == -403 && ra685_apply_fix_blocked() == -403,
        ra680_live_order() == -403,
        ra680_auto_update() == -403,
        true);
}

static const char* title687(int p) {
    switch (p) {
        case FILES: return "V68.1 FILE INTELLIGENCE";
        case TASKS: return "V68.2 TASK INTELLIGENCE";
        case DATA: return "V68.3 DATA INTELLIGENCE";
        case BACKUP: return "V68.4 BACKUP VERIFY";
        case DIAG: return "V68.5 DIAGNOSTICS";
        case CODE: return "V68.6 CODE ADMIN";
        case MASTER: return "V68.7 MASTER REGRESSION";
        default: return "V68.7 FAST MASTER";
    }
}
...
static bool draw687(ANativeWindow* w) {
    ...
    int gap=1*s,bh=12*s,by=2*s,bw=(W-9*gap)/8; const char* bn[8]={"HOME","FILES","TASKS","DATA","BACKUP","DIAG","CODE","MASTER"};
...
static int cb687(int,int,void*) {
    if(!iq687)return 1; AInputEvent*e=nullptr;
    while(AInputQueue_getEvent(iq687,&e)>=0){
        if(AInputQueue_preDispatchEvent(iq687,e))continue; int h=0;
        if(AInputEvent_getType(e)==AINPUT_EVENT_TYPE_MOTION && (AMotionEvent_getAction(e)&AMOTION_EVENT_ACTION_MASK)==AMOTION_EVENT_ACTION_UP){
            float px=AMotionEvent_getX(e,0), py=(float)H687-AMotionEvent_getY(e,0); int s=W687/330; if(s<3)s=3;if(s>5)s=5; int gap=s,bh=12*s,bw=(W687-9*gap)/8;
            if(py>=2*s&&py<=2*s+bh)for(int i=0;i<8;i++){int bx=gap+i*(bw+gap);if(px>=bx&&px<=bx+bw){page687=i;touch687++;h=1;break;}}
        } else if(AInputEvent_getType(e)==AINPUT_EVENT_TYPE_KEY && AKeyEvent_getAction(e)==AKEY_EVENT_ACTION_UP && AKeyEvent_getKeyCode(e)==AKEYCODE_BACK){page687=HOME;touch687++;h=1;}
        AInputQueue_finishEvent(iq687,e,h);
    }
    return 1;
}
...
static void wc687(ANativeActivity*,ANativeWindow*w){win687=w;}
static void wd687(ANativeActivity*,ANativeWindow*){win687=nullptr;}
static void qc687(ANativeActivity*,AInputQueue*q){iq687=q;ALooper*l=ALooper_forThread();if(!l)l=ALooper_prepare(ALOOPER_PREPARE_ALLOW_NON_CALLBACKS);AInputQueue_attachLooper(q,l,1,cb687,nullptr);}
static void qd687(ANativeActivity*,AInputQueue*q){AInputQueue_detachLooper(q);if(iq687==q)iq687=nullptr;}
static void destroy687(ANativeActivity*){run687=false;if(th687.joinable())th687.join();}

} // namespace

extern "C" void ANativeActivity_onCreate(ANativeActivity*a,void*,size_t){
    load687(a);
    a->callbacks->onNativeWindowCreated=wc687;
    a->callbacks->onNativeWindowDestroyed=wd687;
    a->callbacks->onInputQueueCreated=qc687;
    a->callbacks->onInputQueueDestroyed=qd687;
    a->callbacks->onDestroy=destroy687;
    run687=true;
    th687=std::thread(loop687);
}
