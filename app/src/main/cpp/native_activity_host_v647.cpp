...
bool render(ANativeWindow*w){
 ...
 text("VERSION: 64.7",8*s,H-39*s,s,.6f,.75f,.95f);
 ...
 text("MODE: READ ONLY",8*s,H-99*s,s,.75f,.85f,.95f);
 text("SAFETY: LOCKED",8*s,H-113*s,s,.35f,.9f,.55f);
 text("LIVE ORDERS: OFF",8*s,H-127*s,s,.95f,.55f,.3f);
 text("AUTO UPDATE: OFF",8*s,H-141*s,s,.95f,.55f,.3f);
 text("PRODUCTION GATE: CLOSED",8*s,H-155*s,s,.95f,.55f,.3f);
 text("EMERGENCY STOP: READY",8*s,H-169*s,s,.35f,.9f,.55f);
...
void probe(){
 for(int i=0;i<8&&g_alive;i++){
  g_diag=0;g_state=0;redraw();
  bool ok=backend_check();g_state=ok?1:-1;
  LOGI("probe %d state=%d diag=%d",i+1,g_state.load(),g_diag.load());
  redraw();if(ok)break;
...
void on_start(ANativeActivity*){g_alive=true;g_state=0;g_diag=0;}
...
extern "C" int ra647_write(){return-403;}
extern "C" int ra647_apply(){return-403;}
extern "C" int ra647_restore(){return-403;}
extern "C" int ra647_install(){return-403;}
extern "C" int ra647_delete(){return-403;}
extern "C" int ra647_repair(){return-403;}
extern "C" int ra647_live_order(){return-403;}
