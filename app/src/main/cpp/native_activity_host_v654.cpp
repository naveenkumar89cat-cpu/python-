...
bool render(ANativeWindow*w){
 ...
 int s=W/200;if(s<3)s=3;if(s>8)s=8;int st=g_backend_state.load(),dg=g_diag.load();float br=st==1?.2f:.95f,bg=st==1?.9f:.65f,bb=st==1?.35f:.25f;
 int lx=4*s,rx=W/2+2*s,top=H-14*s,row=11*s;
 text("ROBOT ADMIN AI OS",lx,top,s,.92f,.96f,1);text("V65.4 LARGE UI",rx,top,s,.6f,.75f,.95f);
 text("CPP CORE: READY",lx,top-row,s,.35f,.9f,.55f);text("SAFETY: LOCKED",rx,top-row,s,.35f,.9f,.55f);
 text("MODE: OFFLINE FIRST",lx,top-2*row,s,.75f,.85f,.95f);text("WRITE: OFF",rx,top-2*row,s,.95f,.55f,.3f);
 text("PY BACKEND:",lx,top-3*row,s,.92f,.96f,1);text(st==1?"CONNECTED":(st==-1?"OPTIONAL OFF":"CHECKING"),lx+76*s,top-3*row,s,br,bg,bb);text("DELETE: OFF",rx,top-3*row,s,.95f,.55f,.3f);
 text("DIAG: "+std::to_string(dg),lx,top-4*row,s,.8f,.8f,.9f);text("RESTORE: OFF",rx,top-4*row,s,.95f,.55f,.3f);
 text("PROBE OK: "+std::to_string(g_probe_ok.load()),lx,top-5*row,s,.55f,.8f,.95f);text("LIVE ORDERS: OFF",rx,top-5*row,s,.95f,.55f,.3f);
 text("PROBE FAIL: "+std::to_string(g_probe_fail.load()),lx,top-6*row,s,.55f,.8f,.95f);text("AUTO UPDATE: OFF",rx,top-6*row,s,.95f,.55f,.3f);
 text("AUTO RECHECK: ON",lx,top-7*row,s,.55f,.8f,.95f);text("PROD GATE: CLOSED",rx,top-7*row,s,.95f,.55f,.3f);
 text("MODULE MAP: FOUNDATION",lx,top-8*row,s,.55f,.8f,.95f);text("E-STOP: READY",rx,top-8*row,s,.35f,.9f,.55f);
...
void probe_loop(){while(g_alive){bool ok=backend_check();if(ok)g_probe_ok++;else g_probe_fail++;int ns=ok?1:-1;int old=g_backend_state.exchange(ns);if(old!=ns)redraw();for(int i=0;i<50&&g_alive;i++)std::this_thread::sleep_for(std::chrono::milliseconds(100));}g_probe_running=false;}
...
extern "C" const char* ra654_version(){return"65.4";}
extern "C" int ra654_core_ready(){return 1;}
extern "C" const char* ra654_mode(){return"OFFLINE_FIRST";}
extern "C" const char* ra654_safety(){return"LOCKED";}
extern "C" const char* ra654_module_map(){return"FOUNDATION";}
extern "C" int ra654_write_allowed(){return 0;}
extern "C" int ra654_delete_allowed(){return 0;}
extern "C" int ra654_restore_allowed(){return 0;}
extern "C" int ra654_live_orders_allowed(){return 0;}
extern "C" int ra654_auto_update_allowed(){return 0;}
extern "C" int ra654_production_gate_open(){return 0;}
extern "C" int ra654_write(){return-403;}
extern "C" int ra654_apply(){return-403;}
extern "C" int ra654_restore(){return-403;}
extern "C" int ra654_install(){return-403;}
extern "C" int ra654_delete(){return-403;}
extern "C" int ra654_repair(){return-403;}
extern "C" int ra654_live_order(){return-403;}
