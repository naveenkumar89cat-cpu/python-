...
bool backend_check(){
    ...
    if(rc!=0){
        ...
        if(poll(&p,1,1200)<=0){g_diag=2;close(fd);return false;}
...
bool render(ANativeWindow*w){
    ...
    float r=st==1?.2f:.95f,g=st==1?.9f:.65f,b=st==1?.35f:.25f;
    text("ROBOT ADMIN AI OS",8*s,H-25*s,s,.92f,.96f,1);
    text("VERSION: 65.0",8*s,H-39*s,s,.6f,.75f,.95f);
    text("CPP CORE: READY",8*s,H-57*s,s,.35f,.9f,.55f);
    text("MODE: OFFLINE FIRST",8*s,H-71*s,s,.75f,.85f,.95f);
    text("PY BACKEND:",8*s,H-89*s,s,.92f,.96f,1);
    text(st==1?"CONNECTED":(st==-1?"OPTIONAL OFF":"CHECKING"),78*s,H-89*s,s,r,g,b);
    text("DIAG: "+std::to_string(dg),8*s,H-103*s,s,.8f,.8f,.9f);
    text("SAFETY: LOCKED",8*s,H-121*s,s,.35f,.9f,.55f);
    text("WRITE DELETE RESTORE: OFF",8*s,H-135*s,s,.95f,.55f,.3f);
    text("LIVE ORDERS: OFF",8*s,H-149*s,s,.95f,.55f,.3f);
    text("AUTO UPDATE: OFF",8*s,H-163*s,s,.95f,.55f,.3f);
    text("PRODUCTION GATE: CLOSED",8*s,H-177*s,s,.95f,.55f,.3f);
    text("EMERGENCY STOP: READY",8*s,H-191*s,s,.35f,.9f,.55f);
...
void probe(){
    for(int i=0;i<3&&g_alive;i++){
        g_diag=0;g_backend_state=0;redraw();
        bool ok=backend_check();g_backend_state=ok?1:-1;
        LOGI("probe %d state=%d diag=%d",i+1,g_backend_state.load(),g_diag.load());
        redraw();
        if(ok)break;
...
void start_probe(){bool expected=false;if(g_alive&&g_probe_running.compare_exchange_strong(expected,true))std::thread(probe).detach();}

void on_start(ANativeActivity*){g_alive=true;g_backend_state=0;g_diag=0;}
...
extern "C" const char* ra650_version(){return"65.0";}
extern "C" int ra650_core_ready(){return 1;}
extern "C" int ra650_write(){return-403;}
extern "C" int ra650_apply(){return-403;}
extern "C" int ra650_restore(){return-403;}
extern "C" int ra650_install(){return-403;}
extern "C" int ra650_delete(){return-403;}
extern "C" int ra650_repair(){return-403;}
extern "C" int ra650_live_order(){return-403;}
