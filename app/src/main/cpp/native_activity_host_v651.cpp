...
bool backend_check(){
    ...
    bool ok=s.find("200")!=std::string::npos && s.find("PASS")!=std::string::npos;
...
bool render(ANativeWindow*w){
 ...
 text("ROBOT ADMIN AI OS",8*s,H-25*s,s,.92f,.96f,1); text("VERSION: 65.1",8*s,H-39*s,s,.6f,.75f,.95f);
 ...
 text("AUTO RECHECK: ON",8*s,H-117*s,s,.55f,.8f,.95f); text("SAFETY: LOCKED",8*s,H-135*s,s,.35f,.9f,.55f); text("WRITE DELETE RESTORE: OFF",8*s,H-149*s,s,.95f,.55f,.3f);
 text("LIVE ORDERS: OFF",8*s,H-163*s,s,.95f,.55f,.3f); text("AUTO UPDATE: OFF",8*s,H-177*s,s,.95f,.55f,.3f); text("PRODUCTION GATE: CLOSED",8*s,H-191*s,s,.95f,.55f,.3f); text("EMERGENCY STOP: READY",8*s,H-205*s,s,.35f,.9f,.55f);
...
void probe_loop(){
 while(g_alive){
   g_probe_count++; bool ok=backend_check(); int old=g_backend_state.exchange(ok?1:-1); if(old!=g_backend_state.load())redraw();
...
void on_start(ANativeActivity*){g_alive=true;}
...
extern "C" const char* ra651_version(){return"65.1";} extern "C" int ra651_core_ready(){return 1;} extern "C" int ra651_write(){return-403;} extern "C" int ra651_apply(){return-403;} extern "C" int ra651_restore(){return-403;} extern "C" int ra651_install(){return-403;} extern "C" int ra651_delete(){return-403;} extern "C" int ra651_repair(){return-403;} extern "C" int ra651_live_order(){return-403;}
